...
describe("Auth API — Signup", () => {
  ...
  it("POST /api/auth/signup — success creates organizer and returns JWT", async () => {
    ...
    (query as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
      rows: [{ id: "org-1", email: "test@example.com", name: "Test User", avatar_url: null, created_at: "2025-01-01T00:00:00Z" }]
    });
...
describe("Auth API — Login", () => {
  ...
  it("POST /api/auth/login — success returns JWT", async () => {
    const { query } = await import("../lib/db");
    (query as ReturnType<typeof vi.fn>).mockResolvedValueOnce({ rows: [{ id: "org-1", email: "test@example.com", name: "Test User", avatar_url: null, password_hash: "$2b$10$h" }] });
...
describe("Auth API — GET /api/auth/me", () => {
  ...
  it("returns current user", async () => {
    ...
    const { query } = await import("../lib/db");
    (query as ReturnType<typeof vi.fn>).mockResolvedValueOnce({ rows: [{ id: "org-1", email: "test@example.com", name: "Test" }] });
...
describe("Auth API — PATCH /api/auth/me", () => {
  ...
  it("updates and returns organizer", async () => {
    ...
    (query as ReturnType<typeof vi.fn>).mockResolvedValueOnce({ rows: [{ id: "org-1", name: "Updated", email: "test@example.com", created_at: "2025-01-01T00:00:00Z", updated_at: "2025-01-02T00:00:00Z", verified_at: null, avatar_url: null }] });
    const res = await (await import("../pages/api/auth/me")).PATCH({ request: new Request("http://localhost:4321/api/auth/me", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: "Updated" }) }) } as any);
    expect(res.status).toBe(200);
